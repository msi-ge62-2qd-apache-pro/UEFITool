...
void UEFITool::populateUi(const QModelIndex &current)
{
    ...
    // Enable menus
    ui->menuCapsuleActions->setEnabled(type == UEFI::Capsule);
    ui->menuImageActions->setEnabled(type == UEFI::Image);
    ui->menuRegionActions->setEnabled(type == UEFI::Region);
    ui->menuPaddingActions->setEnabled(type == UEFI::Padding);
    ui->menuVolumeActions->setEnabled(type == UEFI::Volume);
    ui->menuFileActions->setEnabled(type == UEFI::File);
    ui->menuSectionActions->setEnabled(type == UEFI::Section);

    // Enable actions
    ui->actionExtract->setDisabled(model->hasEmptyHeader(current) && model->hasEmptyBody(current) && model->hasEmptyTail(current));
    ui->actionRebuild->setEnabled(type == UEFI::Volume || type == UEFI::File || type == UEFI::Section);
    ui->actionExtractBody->setDisabled(model->hasEmptyHeader(current));
    ui->actionRemove->setEnabled(type == UEFI::Volume || type == UEFI::File || type == UEFI::Section);
    ui->actionInsertInto->setEnabled((type == UEFI::Volume && subtype != UEFI::UnknownVolume) || 
    ...
    ui->actionInsertBefore->setEnabled(type == UEFI::File || type == UEFI::Section);
    ui->actionInsertAfter->setEnabled(type == UEFI::File || type == UEFI::Section);
    ui->actionReplace->setEnabled((type == UEFI::Region && subtype != UEFI::DescriptorRegion) || type == UEFI::File || type == UEFI::Section);
    ui->actionReplaceBody->setEnabled(type == UEFI::File || type == UEFI::Section);
}
